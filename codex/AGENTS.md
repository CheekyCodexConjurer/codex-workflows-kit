...
- Long-running jobs have no urgency: start once, do not poll continuously, and consult `status` only under a concrete suspicion that the MCP, worker, or heartbeat failed. Writers use the durable `start_agent` path; `run_agent` is reserved for bounded reader work. Never accelerate, shorten, summarize, cancel, or relaunch a non-terminal job merely because it is slow; wait for its complete result. `cancel` requires explicit cancellation.
