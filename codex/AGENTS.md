...
- Subagents default to GPT-5.4 Mini with xhigh reasoning effort; the native `relay` transport is intentionally low-latency `low` because it only routes the request, while explicit low/high/max variants remain opt-in for analytical roles.
