...
- The internal sub-agent backend is `internal_subagent_backend=opencode` with `internal_subagent_transport=native_relay` in `skills/workflows/references/backend-policy.md`. For each sidecar request, spawn a fresh native `relay` with `multi_agent_v1__spawn_agent(agent_type=relay)`, passing only `{target_agent,cwd,task}`; the relay calls `opencode_worker` using `opencode-go/deepseek-v4-pro`, `max`, and effective no-edit permissions. Do not reuse completed relays or persist/forward MCP session identifiers. The main chat continues useful non-overlap work and joins at the decision/final gate. In the default route, native workers own all writes. Users do not need to add a provider or transport flag to prompts.
