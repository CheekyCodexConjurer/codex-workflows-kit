...
- Subagents default to GPT-5.4 Mini with xhigh reasoning effort; the native `relay` transport uses `high` so it reliably activates the known deferred MCP function, while explicit low/high/max variants remain opt-in for analytical roles.
