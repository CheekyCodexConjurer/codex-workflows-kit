...
- Result-bearing subagents default to GPT-5.6 Luna with max reasoning effort. The native `relay` is a transport-only exception fixed at GPT-5.4 Mini `high` because it reliably activates the deferred MCP tool; it must not be used as the benchmark worker or judge.
