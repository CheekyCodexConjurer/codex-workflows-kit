...
- The OpenCode backend is limited to read-only sidecars through the native `relay`; native workers own all writes, patches, tests, and integration. If OpenCode or the relay is unavailable, preserve the required gate as blocked and never switch providers or permissions silently.
