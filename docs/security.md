...
- A governança de subagentes preserva `subagent_backend` (`native` ou `deepseek`) e
  `subagent_continuation` (`active_follow` ou `park_and_wake`). A execução e a
  revisão são decididas por etapa, respeitando permissões, dependências,
  ownership, capacidade observada e revisão independente. Jev auxilia apenas
  perguntas ambíguas e não concede escrita nem aprova código.
