...
- `preflight-subA`: for non-trivial work, start the quality-first read-only fan-out before/during implementation when independent fronts exist, even without a speed gain; simple tasks stay local, claim-mapped OpenCode workers remain optional, and independent reviewers remain gated by `integrated-freeze`.
