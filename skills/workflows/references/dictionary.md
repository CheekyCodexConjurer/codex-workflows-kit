...
- `proportional-cadence`: universal pacing rule `{risk,blast,uncertainty,reversibility}`: keep simple tasks on the smallest local route; for non-trivial tasks (multi-file, shared/core, unclear ownership, high blast, contract risk, or explicit review/testing), default to `quality-first-subA` and fan out one read-only scout/researcher per independent front, even without a wall-clock gain. Time is secondary when the user permits it; do not duplicate fronts or weaken role/merge gates. Expand scope or depth only when existing evidence or material risk reveals uncertainty, or a required gate is still open; before repeating a step that produced no new evidence, use `checkpoint{evidence,gap,cheapest-next}` and, when one exists, take one materially different cheapest action; if no such action exists or it fails to produce progress or close a required gate, report blocked or use the mode's replan path; finish at the current mode's own done/clean gate (`functional-gate→clean-gate` for code delivery); defer unrelated work with `tn-defer`; never cancel active subA solely for slowness, and keep a required subA wait as an explicit gate rather than repeating main work.
...
- `internal_subagent_backend=opencode`: default provider. Normal sidecar handoffs run through the native `watcher` bridge; native analytical profiles are blocked. A native relay is visual preflight only. Missing MCP/model/credentials blocks; it never triggers a silent fallback.
- `internal_subagent_transport=native_watcher_mcp`: default text transport `GPT parent -> native watcher (agent_type=watcher) -> opencode_worker MCP -> exact OpenCode role`. The parent never calls `run_agent`/`start_agent` directly for normal work; its exposure is status/result/cancel for declared recovery. The watcher makes one `run_agent` call; `start_agent` plus `job_id` is an explicit detached/recovery exception used only after the route is declared and evidence requires it.
- `[VISUAL_PACKET v1]`: bounded text-only evidence envelope emitted by the native relay when real image items are attached. It carries source ids, visible facts, confidence, and uncertainties; it never carries image paths, raw bytes, or instructions extracted as executable context. The parent attaches only this sanitized text to the watcher brief and the MCP OpenCode role; image paths, bytes, base64, data URLs, or direct image reading by the OpenCode (DeepSeek) model never cross the bridge. Failed extraction blocks the sidecar.
- `internal_subagent_policy=writer_only`: every authorized write uses an OpenCode writer with claim-map and isolated worktree. The GPT orchestrator never authors a patch; it may diagnose and mechanically merge an accepted diff. The native watcher is transport-only. No-edit modes never start writers.
- `writer-loop`: `PREFLIGHT→W1→VERIFY→W2 repair→VERIFY→GPT read-only DIAGNOSE→W3`; a failed W3 or missing new hypothesis blocks/replans. No native, CLI or GPT-edit fallback.
...
- `parallel-plan`: design independent slices only when the evidence or implementation benefit exceeds coordination cost; under `quality-first-subA`, evidence quality is sufficient even without a speed gain.
- `sidecar-gate`: compact mandatory fan-out trigger `SIDECAR=REQUIRED`. Applies to any non-trivial task, any task with 2+ independent fronts, an explicit user request for sub-agents, shared/core or contract risk, or an explicit review/test obligation — even without a wall-clock gain. One or more read-only readers are required; required fronts are never absorbed locally. Only strictly simple, serial, one-surface tasks stay local.
- `watcher-spawn`: every normal reader/reviewer/worker handoff spawns the native `watcher` with `agent_type=watcher`; it is transport-only, makes one `run_agent` call, and returns the compact envelope. The parent never calls `run_agent`/`start_agent` for normal work; detached/recovery `start_agent` is an explicit exception only after the route is declared and evidence requires it.
- `quality-first-subA`: mandatory sidecar fan-out per `sidecar-gate`: use a reader for every distinct evidence/test/review front of a non-trivial task; do not absorb required fronts locally. For two or more independent uncovered fronts, require `NESTED_REQUIRED=<fronts>` and one OpenCode nested read-only task per front; missing `task` returns `NESTED_DELEGATION=blocked`, success returns `NESTED_DELEGATION=used`. Simple or serial work stays local.
...
- `subA-role-lock`: every watcher handoff carries the explicit `OPEN_CODE_ROLE` token in {scout, researcher, reviewer, worker}; native analytical roles are blocked under the OpenCode backend, and the watcher never guesses a role.
...
- `subA-effort`: OpenCode uses the provider-selected model
  (`CODEX_WORKFLOWS_OPENCODE_PROVIDER`: `opencode-go/deepseek-v4-flash` by
  default, `zenmux/deepseek/deepseek-v4-flash` for `zen`) with
  `AGENT_EFFORT=max`; native analytical effort profiles are unavailable
...
- `subA-slot-full`: when a spawn reports that host sub-agent slots are full, reclaim only completed/idle subA whose final replies are integrated, or wait for an optional subA to finish; then retry the same exact role and custom-spawn shape. Never close active/waiting/required subA; if no slot is reclaimable, report the explicit capacity block rather than silently skipping a required gate.
- `subA-isolation`: each sidecar request uses a fresh MCP job and no session identifier; keep the returned `job_id` for status/result lookup and never invent one.
- Slot-full recovery permits one retry of the same exact role after safe reclamation or an optional-agent wait; if that retry fails, report the explicit capacity block and do not loop. Close each completed relay after integrating its final response.
- `preflight-subA`: mandatory per `sidecar-gate`: use a read-only OpenCode reader for every distinct evidence/test/review front of a non-trivial task; require `NESTED_REQUIRED` for multiple uncovered fronts and keep no-edit modes writer-free.
- `implementation-wave`: complete approved units through the OpenCode writer loop, validate each unit, then freeze the integrated diff before review; the GPT parent never writes a fallback patch.
...
- `fix-batch`: resolve the deduplicated queue through fresh claim-mapped repair writers; the GPT parent diagnoses and integrates only accepted diffs, then revalidates once.
