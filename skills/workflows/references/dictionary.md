...
- `internal_subagent_backend=opencode`: default provider. Use the exposed `opencode_worker` MCP directly; native analytical profiles are blocked. A native relay is visual preflight only. Missing MCP/model/credentials blocks; it never triggers a silent fallback.
- `internal_subagent_transport=direct_mcp`: default text transport. Use `run_agent` only for a bounded smoke and `start_agent` plus `job_id` for long work; consult status at a prolonged-wait or decision checkpoint.
...
- `internal_subagent_policy=writer_only`: every authorized write uses an OpenCode writer with claim-map and isolated worktree. The GPT orchestrator never authors a patch; it may diagnose and mechanically merge an accepted diff. No-edit modes never start writers.
...
- `quality-first-subA`: use a reader only for a distinct evidence/test/review front. For two or more independent uncovered fronts, require `NESTED_REQUIRED=<fronts>` and one OpenCode nested read-only task per front; missing `task` returns `NESTED_DELEGATION=blocked`. Simple or serial work stays local.
...
- `subA-role-lock`: direct MCP requests select the exact OpenCode role `scout`, `reviewer`, `researcher`, or `worker`; native analytical roles are blocked under the OpenCode backend.
...
- `subA-effort`: OpenCode uses the configured `opencode-go/deepseek-v4-flash`
  with `AGENT_EFFORT=max`; native analytical effort profiles are unavailable
...
- `preflight-subA`: use a read-only OpenCode reader only for an explicit evidence/test/review front; require `NESTED_REQUIRED` for multiple uncovered fronts and keep no-edit modes writer-free.
