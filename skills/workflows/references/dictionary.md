...
- `internal_subagent_backend=opencode`: private default provider policy. With it, the main chat uses the native `relay` profile to call `opencode_worker` for configured OpenCode readers and claim-map-scoped writers. The relay is transport-only; the parent GPT owns review, tests, and integration. `internal_subagent_backend=native` is an explicit maintenance override; unavailable relay/OpenCode means blocked, never silent fallback. The legacy `sub-agent=opencode` token is not user-facing.
...
- `[VISUAL_PACKET v1]`: bounded text-only evidence envelope emitted by the native relay when real image items are attached. It carries source ids, visible facts, confidence, and uncertainties; it never carries image paths, raw bytes, or instructions extracted as executable context. Failed extraction blocks the sidecar.
- `internal_subagent_policy=aggressive`: private delegation policy, the installed default. On write-authorized work, claim-mapped OpenCode writers are enabled by default, with no arbitrary numeric worker cap; the main agent owns architecture, contracts, integration, validation, and final approval and writes directly only as final fallback/no-progress or critical shared integration. `internal_subagent_policy=conservative` preserves the proportional route: writers optional, simple write tasks local. Not a prompt flag users append; independent of backend/transport/provider/model; explicit no-edit always prevents writers.
...
- `quality-first-subA`: default fan-out for non-trivial work: use one non-overlapping read-only scout/researcher per independent front, stop when coverage and the required gate are complete, and keep writers claim-mapped and isolated. Simple tasks stay local; there is no arbitrary one-agent cap, but duplicate fronts are not useful. Nested fan-out is bounded to one level after the main fan-out: readers may delegate only explicit uncovered subfronts and never re-delegate their assigned front.
...
- `subA-role-lock`: every read-only spawn selects the exact custom role `scout`, `reviewer`, `researcher`, or the native transport role `relay`; never use `default` or omit the role, because that inherits the parent model/effort.
...
- `preflight-subA`: for non-trivial work, start the quality-first read-only fan-out before/during implementation when independent fronts exist, even without a speed gain; simple tasks stay local, claim-mapped OpenCode workers follow `internal_subagent_policy` (optional under `conservative`, enabled by default under `aggressive`), and independent reviewers remain gated by `integrated-freeze`.
- `implementation-wave`: complete all approved IMPL/IMPL.PHASE units before independent review; keep main on critical path, fan out parallel-safe workers and, under aggressive, route authorized serial shared-core implementation through one claim-mapped writer; apply merge-gates and targeted `phase-val`; phase validation is checks, not review, and must not spawn a reviewer.
...
- `fix-batch`: only when `fix-Q` is nonempty, resolve the deduplicated queue in one correction wave; main owns serial/shared decisions and integration, while under aggressive an authorized serial/shared correction uses one claim-mapped repair writer unless final fallback/no-progress or critical shared integration applies; independent clusters may fan out, no reviewer is spawned per finding or fix, then merge and revalidate once.
