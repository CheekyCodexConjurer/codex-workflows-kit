...
- **Economia de Tokens na Delegação (Token Economy & Session Continuity)**:
  ...
  - **Projeção Compacta de Follow**: O retorno de `subagents_follow` entrega uma carga estruturada leve (~1-2KB) com `claims` delimitadas (resumo <= 1.000 caracteres, até 10 arquivos, até 10 testes, até 5 riscos), tokens consumidos no turno e `detailsRef`, omitindo o dump maciço de `progress` e o envelope bruto não sanitizado da resposta padrão.
  - **Recuperação Sob Demanda Paginada**: A ferramenta `subagents_recover_result` suporta parâmetros `section` (`summary`, `files`, `tests`, `risks`, `diff`, `evidence`, `full`), `offset` e `limit`, permitindo que o parent consulte seletivamente apenas as partes do resultado indispensáveis para a próxima decisão sem poluir a janela de contexto.
  - **Telemetria Estruturada e Semântica Não-Cumulativa de Delta**: Métricas de uso de tokens do worker (`worker_input_tokens`, `worker_output_tokens`, `worker_thinking_tokens`, `worker_cached_input_tokens`, `worker_total_tokens`) são persistidas por job/tentativa. Como o `agy.exe` acumula tokens entre múltiplos turnos na mesma conversa (`--conversation`), o bridge calcula deterministicamente o delta incremental de cada turno (`delta = max(0, usage_atual - soma_turnos_anteriores)`), prevenindo dupla contagem e expondo valores precisos em `FollowResult.tokens`.
