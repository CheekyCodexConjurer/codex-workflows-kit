...
no-edit; the claim-map-scoped OpenCode `worker` may edit only its supplied
  ...
  relay transport-only except for the bounded visual preflight, and keep the
  parent responsible for tests and integration.

The installed `internal_subagent_policy=aggressive` default enables
claim-mapped OpenCode writers on write-authorized work, with no arbitrary
numeric worker cap; the parent owns architecture, contracts, integration,
validation, and final approval, and writes directly only as final
fallback/no-progress or critical shared integration.
`internal_subagent_policy=conservative` keeps writers optional and simple
write tasks local. The policy is private, orthogonal to
backend/transport/provider/model, and an explicit no-edit always prevents
writers.

For image-bearing sidecars, when the host supports real native image items,
attach them to the relay spawn and keep the text task free of image paths. The
relay performs the native visual preflight and sends the bounded task text plus
only the `[VISUAL_PACKET v1]` block to `opencode_worker`. If items were attached
but the relay returns `RELAY_VISUAL=none` or omits the status, treat the result
as blocked/unknown; a failed visual extraction is never a path-only fallback.
...
- `PLAN`: use `tn-observe`. Simple no-edit plan. Simple tasks stay local; for non-trivial plans, use the `quality-first-subA` read-only fan-out across independent fronts even without a speed gain, then return the ordered plan with files/risks/deps/validation and compact `delivery-contract`; add parallel slices only when `parallel-safe`.
...
- `IMPL`: use `tn-enforce`. Implement approved scope. Use plan-cache/memory-gate, direct reads, the `quality-first-subA` read-only preflight for non-trivial independent fronts, optional `subA-gate`, smallest safe diff, `rework-checkpoint`, size-check, verify-tier, and `quality-delta`; final changes/files/validation/risks.
- `IMPL.PHASE`: use `tn-enforce`. Implement approved phased roadmap. Consume phase graph/claim-map draft and `quality-obligations` if present. Execute `earned-rework-approved` only inside approved contracts. Main owns critical path. Start the `quality-first-subA` read-only preflight for non-trivial independent fronts; fan out writable sidecars only when parallel-safe, and under aggressive serialize an authorized shared-core implementation through one claim-mapped writer. Execute unit phases, validate each, checkpoint, continue only if clean.
- `DELIVER.AUTO`: use `tn-enforce` and execute only approved `quality-obligations`; broad discoveries trigger `replan-gate`. Consume an approved `delivery-contract` and route to `IMPL` or `IMPL.PHASE`. Start the `quality-first-subA` read-only preflight for non-trivial work across independent fronts even without a speed gain; simple tasks stay local, and claim-mapped OpenCode workers are optional under `conservative` and enabled by default under the installed `aggressive` policy. Then complete the full approved `implementation-wave`: main keeps the critical path moving, workers handle claim-map-scoped isolated-worktree slices through the native relay; fan out only parallel-safe slices, and under aggressive serialize an authorized shared-core implementation through one claim-mapped writer. Each unit gets merge-gate plus targeted `phase-val`; phase validation does not spawn reviewers. Under the internal backend policy, the relay remains transport-only, the parent runs integrated tests, and the native `worker` profile is only an explicit maintenance override. After every approved phase and worker is complete, pass `integrated-freeze` and only then run integrated validation plus the risk-tiered `review-batch` in parallel on one stable snapshot. Every read-only spawn uses its exact custom role and every writer request uses the `worker` target with a claim-map; every relay request omits `fork_context`, `model`, and `reasoning_effort`; never combine `fork_context=true` with `agent_type`. A transient availability error gets one fresh retry of that same valid custom-spawn shape after useful local work, never a fallback to `default`, and an unavailable required reviewer leaves the review gate blocked. Triage once through `finding-gate`; if no actionable finding remains and acceptance passes, go directly to `clean-gate`. Otherwise execute one batched `fix-batch`, merge and revalidate, then run `delta-closure`; do not spawn reviewers per phase, finding, or individual fix, and repeat full reviewer lenses only after a material risk-surface change. Use `early-review-exception` only for an explicit high-impact contract/security/data/migration risk or unresolved validation failure. `clean-gate` completes delivery; `replan-gate` stops/escalates. No commit.
...
- `COMMIT`: use `tn-verify`; never start feature or structural refactoring here. Inspect status/index/diff/untracked/remotes, classify every candidate including staged content, and run `gitignore-hygiene`; block on suspected staged secret/generated/cache/local material or broad existing ignores of likely source/docs/config. Keep simple commits local; for non-trivial independent candidate fronts, use the `quality-first-subA` read-only scout fan-out; Git/index/commit/push remain parent-owned. Run `commit-coverage-gate` before each commit and before push. Build `commit-map` units by independently revertible behavior rather than file or agent. Preserve coherent pre-staged content as an explicit unit; `commit-series=auto` validates and commits ordered units with conventional title, Context, Validation, and `Operator` trailer. Run integrated validation, then `push=current` only when the current remote branch already exists, without force, branch creation, or upstream changes; preserve local commits and report when validation or push blocks.
- `BUG.INV`: use `tn-observe`. No-edit, evidence-first investigation. Define repro/failure signature, use the `quality-first-subA` read-only fan-out across non-trivial independent fronts/hypotheses, use ctx-loop/CodeGraph if useful, prove or reject hypotheses one by one, distinguish observation/inference/unknown, and finish with root-cause evidence plus a minfix plan and causal `quality-obligations`; when an evidence gap remains material, record the `obs-gate` result for the next approved unit without instrumenting here.
