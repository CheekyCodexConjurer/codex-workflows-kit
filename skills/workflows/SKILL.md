...
- Before material FANOUT, run the Jev delegation gate in `scripts/subagent-gate.ps1` with a small metadata-only input and the existing policy decision. Its default is `on` (`CODEX_SUBAGENT_JEV_MODE=off|shadow|on`); `shadow` records a recommendation without changing execution. Deterministic rules run first. The gate chooses parent, same-route worker reuse, one front, or independent batch; it never chooses the backend. See `references/delegation.md` for the exact contract.
...
- After a worker result and deterministic checks, run the Jev review gate in `scripts/subagent-gate.ps1`. It may omit only an extra parent semantic reading of an obviously safe result. Failure, uncertainty, scope drift, architecture, or missing evidence always require GPT review. It never waives the independent delivery review or any workflow gate (`references/delivery-review.md`).
