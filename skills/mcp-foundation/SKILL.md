...
| CodeGraph | Structural architecture, symbol relationships, and call flow | .codegraph exists in repository root or explicit projectPath | Structural first via codegraph_explore; direct rg for literal errors/terms; status -> sync -> recheck; fall back to Serena/rg; never auto-init |
...
- Use CodeGraph for structural exploration in medium-to-large codebases to trace module dependencies, architecture, and multi-file call paths before broad file reads. Direct textual searches for exact error messages, configuration keys, or literal strings should use `rg` directly without graph indirection. Structural queries must invoke `codegraph_explore` directly. Curated task context (candidate files, relevant symbols, and error context) should be shared from the parent to sub-agents to avoid redundant discovery from the project root.
