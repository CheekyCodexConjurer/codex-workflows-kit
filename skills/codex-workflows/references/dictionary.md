...
- `subA-effort`: select the custom profile `<role>-<effort>` when work needs `low`, `high`, `xhigh`, or `max` reasoning; the unsuffixed role is the `medium` baseline. Every profile uses 5.6 Sol.
